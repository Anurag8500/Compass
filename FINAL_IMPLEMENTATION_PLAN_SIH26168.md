...
- **Cadence & Causality**: Explicit timeline-anchored scheduler enforces ~2 Hz VelocityNet ($\Delta t = 0.5\text{ s}$) and ~1 Hz BiasNet ($\Delta t = 1.0\text{ s}$) with a 20 ms jitter tolerance ($\delta_{\text{tol}} = 0.02\text{ s}$). Grid anchors prevent drift from accumulating over time under real IMU timestamp jitter. Diagnostic telemetry strictly distinguishes and reconciles: `scheduler_due == buffer_not_ready + inference_executed` and `inference_executed == update_accepted + update_rejected`. Warm-up accounts 4 due epochs for VelocityNet and 2 due epochs for BiasNet as `buffer_not_ready` without 10 Hz retry loops. `CausalWindowBuffer` strictly prevents lookahead ($t_i \le t_{\text{update}}$).
...
- **Controlled Real-Data Replay (`Categorised_S1.npz`, Segment-Local ENU Frame)**:
  ...
  - **Scenario Suite Evaluated**:
    1. `continuous_gnss_sanity` (60s, moving at 14.1 m/s, 842.2m traveled): Validates GNSS/ESKF tracking and filter stability. Pure ESKF: RMSE $0.514\text{ m}$, Final H $0.152\text{ m}$, Vel RMSE $0.486\text{ m/s}$; +VNet+BNet: RMSE $0.719\text{ m}$, Final H $0.819\text{ m}$, Vel RMSE $0.477\text{ m/s}$; 60/60 GNSS fixes applied. Cadence: VNet 120 due (4 not_ready + 116 executed [116 acc, 0 rej]); BNet 60 due (2 not_ready + 58 executed [58 acc, 0 rej]).
    2. `moving_outage_10s` (10s, moving at 13.4 m/s, 133.9m traveled): Pure ESKF Final H $13.836\text{ m}$, Vel RMSE $1.874\text{ m/s}$; +BNet Final H $12.954\text{ m}$ (BiasNet reduces drift by $0.88\text{ m}$); +VNet+BNet Final H $20.560\text{ m}$, Vel RMSE $2.628\text{ m/s}$. Cadence: VNet 20 due (4 not_ready + 16 executed [16 acc, 0 rej]); BNet 10 due (2 not_ready + 8 executed [8 acc, 0 rej]).
    3. `moving_outage_30s` (30s, moving at 13.9 m/s, 417.5m traveled): Pure ESKF Final H $106.423\text{ m}$, Vel RMSE $4.566\text{ m/s}$; +VNet+BNet Final H $257.389\text{ m}$, Vel RMSE $3.644\text{ m/s}$ (-20.2% velocity error reduction). Cadence: VNet 60 due (4 not_ready + 56 executed [56 acc, 0 rej]); BNet 30 due (2 not_ready + 28 executed [28 acc, 0 rej]).
    4. `moving_outage_60s` (60s, moving at 14.1 m/s, 842.2m traveled): Pure ESKF Final H $753.808\text{ m}$, Vel RMSE $23.239\text{ m/s}$; +VNet+BNet Final H $459.391\text{ m}$ (-294.42m drift reduction!), Vel RMSE $5.770\text{ m/s}$ (-75.2% error reduction!). Cadence: VNet 120 due (4 not_ready + 116 executed [116 acc, 0 rej]); BNet 60 due (2 not_ready + 58 executed [58 acc, 0 rej]).
    5. `sharp_turn_stress` (60s, turning segment, 238.2m traveled): Retained as diagnostic stress test. 27/60 GNSS fixes applied. In ground truth, an 84° turn begins at $t_{\text{trip}} = 51.5\text{s}$ ($t_{\text{rel}} = 26.5\text{s}$), but sensor angular velocity is delayed and recorded primarily in the phone pitch axis (as stationary calibration determined `is_yaw_aligned: False`). At $t_{\text{rel}} = 28.0\text{s}$ ($t_{\text{trip}} = 53.0\text{s}$), the position innovation reaches $18.73\text{m}$, evaluating to $\text{NIS} = 19.76 > 11.345$ ($\chi_3^2(0.99)$), triggering outlier rejection. Without Phase 10's persistent-rejection reacquisition logic, open-loop dead reckoning ensues for the remaining 33 fixes. Cadence: VNet 120 due (4 not_ready + 116 executed [9 acc, 107 rej]); BNet 60 due (2 not_ready + 58 executed [58 acc, 0 rej]).
...
- **Test Suite Accounting**:
  - Full Test Suite: `uv run pytest` $\to$ **343 collected, 343 passed, 0 failed, 0 skipped, 14 warnings in 27.23s**.
  - Phase 9 Core & Integration Suite: `uv run pytest tests/unit/test_ml_measurements.py tests/integration/test_ml_eskf_authority.py tests/integration/test_ml_active_without_gnss.py tests/integration/test_full_ml_replay_real_data.py` $\to$ **34 collected, 34 passed in 3.42s**.
...
- [x] Current-fix pre-update innovation NIS (`current_measurement_nis`) evaluated against unscaled baseline noise covariance $\mathbf{R}_{\text{base}}$ before computing trust score, eliminating 1-fix lag and avoiding circular logic. Continuous scaling $\mathbf{R}_{\text{eff}} = \frac{1}{\max(S_{\text{trust}}, 0.05)} \mathbf{R}_{\text{base}}$ applied inside `GNSS_AIDED`. Phase 5 ESKF innovation gating preserved.
- [x] Authoritative 3-state FSM (`GNSS_AIDED ⇄ DR_ONLY ⇄ REACQUIRING`) implemented in `navigation/gnss/fsm.py` with dwell time hysteresis ($T_{\text{dwell}} = 2.0\text{ s}$), authoritative timeout ($T_{\text{reacq}} = 10.0\text{ s}$), and zero discrete duration states (no DEGRADED state).
- [x] Timestamp-based outage detector implemented in `navigation/gnss/outage_detection.py` with $2.0\text{ s}$ grace period and $3.0\text{ s}$ timeout, distinguishing missing fixes from rejected fixes. Strict acceptance accounting (`update_applied = bool(diag_p is not None and diag_p.applied)`) ensures unconverged recovery fixes are not misclassified as accepted GNSS updates.
- [x] Bounded-rate recovery implemented in `navigation/gnss/recovery.py` enforcing $v_{\text{blend}} \le 2.0\text{ m/s}$ and max single step $\le 3.0\text{ m}$; requiring 3 consecutive convergence fixes ($\le 1.5\text{ m}$) before returning to `GNSS_AIDED` along the canonical traversal `DR_ONLY → REACQUIRING → GNSS_AIDED`, and aborting back to `DR_ONLY` on implausible returning fix. Explicit supervisory nominal position smoothing without covariance mutation (leaving $P$ unmodified; normal GNSS ESKF updates remain authoritative upon convergence).
- [x] ML models (VelocityNet and BiasNet) remain 100% active during `DR_ONLY`.
- [x] Test suite: 37 Phase 10 unit and integration tests passing; Phase 9 regressions 34/34 passing; full repository test suite 384/384 passing (0 failed, 14 warnings in 37.24s).
- [x] Empirical results documented in `docs/outage_recovery_report.md` and `docs/outage_recovery_results.json`. Real data strictly classified as `REAL_DATA_REPLAY` (no verified natural outage index exists in IO-VNBD v1).
