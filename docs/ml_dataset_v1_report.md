...
$$\mu_c = \frac{1}{M_{\text{train}} \times 20} \sum_{m=1}^{M_{\text{train}}} \sum_{t=1}^{20} X_{m, t, c}, \quad \sigma_c = \sqrt{\frac{1}{M_{\text{train}} \times 20} \sum_{m=1}^{M_{\text{train}}} \sum_{t=1}^{20} (X_{m, t, c} - \mu_c)^2}$$

### 5.1 Exact Empirical Statistics (from `data/ml_dataset_v1/normalization.json`)

| Channel | Name | Training Mean ($\mu$) | Training Std ($\sigma$) | Physical Interpretation |
|---|---|---|---|---|
| 0 | `f_x_v` | $-0.0692\,\text{m/s}^2$ | $1.3782\,\text{m/s}^2$ | Longitudinal vehicle acceleration/braking |
| 1 | `f_y_v` | $+0.0461\,\text{m/s}^2$ | $1.2959\,\text{m/s}^2$ | Lateral cornering acceleration |
| 2 | `f_z_v` | $+9.8382\,\text{m/s}^2$ | $0.5654\,\text{m/s}^2$ | Vertical specific force (centered on Earth gravity) |
| 3 | `omega_x_v` | $+0.000067\,\text{rad/s}$ | $0.1196\,\text{rad/s}$ | Vehicle roll angular rate |
| 4 | `omega_y_v` | $+0.003219\,\text{rad/s}$ | $0.2473\,\text{rad/s}$ | Vehicle pitch angular rate |
| 5 | `omega_z_v` | $+0.000009\,\text{rad/s}$ | $0.1211\,\text{rad/s}$ | Vehicle yaw angular rate |
| 6 | `norm_f_v` | $+10.0156\,\text{m/s}^2$ | $0.6193\,\text{m/s}^2$ | Specific force vector Euclidean norm |
| 7 | `norm_f_dot_v` | $+9.4468\,\text{m/s}^3$ | $8.8077\,\text{m/s}^3$ | Causal backward jerk magnitude |
| 8 | `norm_omega_v` | $+0.2219\,\text{rad/s}$ | $0.2022\,\text{rad/s}$ | Angular velocity vector Euclidean norm |
...
## 6. VelocityNet Ground-Truth Labels

- **Target Definition**: Vehicle forward speed in $\text{m/s}$ at the exact **window-end timestamp** $T$:
  $$\text{target} = v(T)$$
- **Reference Source**: Racelogic VBOX RTK ground truth (`v_ref_speed_mps`).
- **Causal Median Smoothing**: A short median filter ($W = 3$ samples over $[t_{k-2}, t_{k-1}, t_k]$) is applied before label assignment to attenuate discretization quantization while preserving genuine motion steps without future lookahead.
- **Dual Provenance**: Both smoothed targets (`y_speed`) and raw reference speeds (`y_speed_raw`) are serialized side-by-side in each `.npz` archive.
...
- **Result**: The plumbing safely flags all 371,472 valid windows as `ELIGIBLE` with reason `ELIGIBLE`. When real outage metadata is supplied in future benchmarks, the interface will automatically exclude overlapping windows without requiring code changes.
...
|---|---|---|
| `train.npz` | ~400 MB | 233,830 windows: `X`, `X_raw`, `y_speed`, `y_speed_raw`, timestamps, file IDs, driver IDs |
| `validation.npz` | ~72 MB | 42,388 windows: `X`, `X_raw`, `y_speed`, `y_speed_raw`, timestamps, file IDs, driver IDs |
| `test.npz` | ~210 MB | 123,496 windows: `X`, `X_raw`, `y_speed`, `y_speed_raw`, timestamps, file IDs, driver IDs |
| `normalization.json` | 1.1 KB | Serialized means, stds, sample count, channel order |
...
- **Phase 6 ML Dataset Unit Tests**: 34 passed
  - `test_resample.py`: 5 passed
  - `test_features.py`: 4 passed
  - `test_windowing_causality.py`: 4 passed
  - `test_split.py`: 5 passed
  - `test_leakage_audit.py`: 4 passed
  - `test_normalization_train_only.py`: 4 passed
  - `test_velocitynet_labels.py`: 4 passed
  - `test_exclusion_rules.py`: 4 passed
- **Phase 6 ML Dataset Integration Tests (`test_ml_dataset_artifacts.py`)**: 5 passed
- **Total Tests**: **241 passed, 0 failures, 0 errors**.

### 9.2 Reproducibility Result
The complete dataset build was executed twice independently from cold processes.
- Manifest match: `True`
- Maximum difference in normalization means: `0.0`
- Maximum difference in normalization stds: `0.0`
