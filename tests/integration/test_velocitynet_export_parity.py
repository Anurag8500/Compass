...
def test_export_parity_v1_1_500_windows_acceptance():
    """Acceptance test verifying numerical parity for VelocityNet v1.1 across 500 real test windows.
    ...
    project_root = Path(__file__).resolve().parents[2]
    test_npz = project_root / "data" / "ml_dataset_v1" / "test.npz"
    onnx_path = project_root / "models" / "velocitynet_v1_1.onnx"
    tflite_path = project_root / "models" / "velocitynet_v1_1.tflite"
    ckpt_path = project_root / "models" / "velocitynet_v1_1_best.pt"

    if not (test_npz.exists() and onnx_path.exists() and tflite_path.exists() and ckpt_path.exists()):
        pytest.skip("Required v1.1 model artifacts or test dataset not found for 500-window acceptance test")
    ...
    assert parity_report["onnx_parity"]["passed"] is True, f"v1.1 ONNX parity failed: {parity_report['onnx_parity']}"
    assert parity_report["onnx_parity"]["speed_max_abs_err"] <= 1e-4
    assert parity_report["onnx_parity"]["log_var_max_abs_err"] <= 1e-4

    assert parity_report["litert_parity"]["passed"] is True, f"v1.1 LiteRT parity failed: {parity_report['litert_parity']}"
    assert parity_report["litert_parity"]["speed_max_abs_err"] <= 1e-3
    assert parity_report["litert_parity"]["log_var_max_abs_err"] <= 1e-3
    assert parity_report["all_passed"] is True
